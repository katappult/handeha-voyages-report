...
from app.utils.utils import get_global_search_theme


class GlobalSearchThemes(Resource):
    def get(self):
        """
        Get global_routes search theme data based on a date range.

        This endpoint retrieves global_routes search theme data between a given `start_date` and `end_date`.
        ...
        responses:
            200:
                description: A list of global_routes search theme data between the given date range
                content:
                    application/json:
                        schema:
                            type: array
                            items:
                                ...
                                properties:
                                    theme:
                                        type: string
                                        description: The theme associated with the search
                                    count:
                                        type: integer
                                        description: The number of searches conducted on that theme
                        example:
                            - theme: "Technology"
                              count: 200
                            - theme: "Health"
